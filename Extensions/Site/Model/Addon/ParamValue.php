...
/**
 * @property int addon_param_value_id
 * @property int addon_param_id
...
{
    protected static $table = 'catalog_addon_param_value';
    protected static $primaryKeyName = 'addon_param_value_id';

    public function offsetGetParam()
    {
        return Param::findOne(['addon_param_id' => $this->addon_param_id]);
    }

    public static function getList(int $addonId): array
    {
        $cache = self::find(['addon_id' => $addonId]);

        $out = [];
        foreach ($cache as $val) {
            $out[$val->addon_param_id] = $val->value;
        }
     ...
     * @throws \Exception
     */
    public static function updateForProduct(int $addonId): void
    {
        ...
        foreach ($req->post() as $k => $v) {
            ...
            $item = self::findAdv()->where(
                ['addon_id' => $addonId, 'addon_param_id' => $paramId]
            )->fetchOne();
            if ($item) {
                ...
                // create new
                $item = new self;
                $item->addon_id = $addonId;
                $item->addon_param_id = $paramId;
                $item->value = trim($v);
                $item->save();
